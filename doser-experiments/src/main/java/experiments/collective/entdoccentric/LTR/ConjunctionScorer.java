...
class ConjunctionScorer extends Scorer {
	...
	public ConjunctionScorer(Weight weight, int docBase, Collection<Scorer> scorers, LearnToRankClause[] learnToRankclauses)
			throws IOException {
		this(weight, docBase, learnToRankclauses, scorers.toArray(new Scorer[scorers.size()]));
	}
	...
	public float score() throws IOException {
		...
		float sum = 0.0f;
		for (int i = 0; i < scorers.length; i++) {
			float val = scorers[i].score() * clauses[i].getWeight();
			sum += val;
