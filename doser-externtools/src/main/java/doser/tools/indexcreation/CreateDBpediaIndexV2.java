	...
	public void createNewIndex() {
		...
		try {
			...
			for (String uri : entities) {
				...
				// Add Wikipedia Text
				doc.add(new TextField("Wikipedia", generateWikipediaText(uri), Field.Store.YES));
...
//			throws QueryException, QueryParseException {
	...
	public String generateWikipediaText(String url) {
		String wikitext = new String();
		String urlending = url.replaceAll("http://dbpedia.org/resource/", "");
		File f = new File("/mnt/storage/zwicklbauer/WikiParse/temp/plain_reduced/"+urlending);
		String filecontent = "";
		try {
			BufferedReader reader = new BufferedReader(new FileReader(f));
			String line = null;
