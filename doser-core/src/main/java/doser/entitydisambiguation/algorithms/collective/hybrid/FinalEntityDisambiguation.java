...
public class FinalEntityDisambiguation extends Word2VecPageRank {

	private static final int PREPROCESSINGCONTEXTSIZE = 500;

	private Doc2Vec d2v;

	public FinalEntityDisambiguation(EnCenExtFeatures featureDefinition,
		...
		for (Vertex v1 : vertexList) {
			for (Vertex v2 : vertexList) {
				if (!v1.equals(v2) && !areCandidatesofSameSF(v1, v2)) {
					...
					List<String> l2 = v2.getUris();
					if (l1.size() == 1 && l2.size() == 1) {
						double weight = similarityMap.get(this.w2v
