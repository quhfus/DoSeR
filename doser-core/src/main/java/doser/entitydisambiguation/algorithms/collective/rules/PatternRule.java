...
public class PatternRule extends Rule {
	...
	private static final float OCCURRENCEPERCENTAGE = 0.45f;
	...
	private static final int MAXSURFACEFORMS = 4; 
	
	public PatternRule(EntityCentricKnowledgeBaseDefault eckb) {
		super(eckb);
	}
	...
	private Map<String, Integer> generateDictionary(List<SurfaceForm> rep) {
		...
		for (SurfaceForm sf : rep) {
			...
			for (String str : strList) {
				String ending = str.replaceAll("http://dbpedia.org/resource/",
						"").toLowerCase();
				String[] split = ending.split("_");
				if (split.length > 1) {
					for (int i = 1; i < split.length; i++) {
						if (!customStopWordList.contains(split[i]) && m.get(s) < MAXSURFACEFORMS) {
	...
	private void disambiguateTerms(String str, List<SurfaceForm> rep) {
		for (SurfaceForm sf : rep) {
			...
			for (String s : l) {
				String st = s.replaceAll("http://dbpedia.org/resource/", "")
						.toLowerCase();
				if (st.contains(str)) {
					candidates.add(s);
				}
	...
	public static void main(String[] args) {
		...
		SurfaceForm sf3 = new SurfaceForm("Essex", "", l3, 0, 1);
