...
public class EntitySignificanceAlgorithmPR_W2V implements
	...
	public String process(Map<DisambiguatedEntity, Integer> map) {
		...
		PageRank<Vertex, Edge> pr = new PageRank<Vertex, Edge>(
				graph, MapTransformer.getInstance(edgeWeights), 0.1);
	...
	private void computeWord2VecSimilarities(Set<String> entities) {
		...
		for (String s1 : entities) {
			for (String s2 : entities) {
				combinations.add(s1.replaceAll("http://dbpedia.org/resource/", "") + "|" + s2.replaceAll("http://dbpedia.org/resource/", ""));
			}
		}
	...
	private DirectedGraph<Vertex, Edge> buildGraph(
		...
		for (Vertex v1 : vertexes) {
			for (Vertex v2 : vertexes) {
				float similarity = this.getWord2VecSimilarity(v1.getUris().get(0),
						v2.getUris().get(0));
